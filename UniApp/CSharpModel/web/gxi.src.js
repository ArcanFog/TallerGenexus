	...
	fixes: (function () {
		...
		return {
			...
			fixTableResets: function (root) {
				...
				gx.dom.setStyleElement(styleEl, cellPaddingsTemplate(newCellPaddings) + cellSpacingsTemplate(newCellSpacings), true);
			},
...
gx.evt_i = (function($) {
	...
	fireControlValueChange: function(gxO, Ctrl, evt) {
		...
		if (vStruct && vStruct.evt_cvc) {
			...
			return gxO[vStruct.evt_cvc].call(gxO).always( function () {						
				...
				gx.evt.endValidation(vStruct.gxGrid, true);
			});					
		}
		return $.Deferred().resolve();
	},
	...
	endValidation: function (gxgrid, skipNotify) {
...
gx.ajax = (function ($) {
	...
	return {
		...
		newRows: function (CmpCtx, InMasterPage, GridName, gxRows, gxRowIndex, gxRowId, gxParentRow, gHandler) {
			...
			var gridObj = gx.fn.gridObj(CmpCtx, (gxParentRow) ? (GridName + '_' + gxParentRow) : GridName, InMasterPage);
			
			parameters.push('gxajaxNewRow_' + GridName);
			...
			sURL += gx.ajax.encryptParms(gxO, parameters.join(','));
